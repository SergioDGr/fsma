...
<template>
    ...
    <router-link :to="{ name:'Serie',params:{id: id} }" >
        <img v-if="!image.includes('/w500null')" class="rounded" :src="image" />
        <img v-else class="rounded" :src="image_default" />
    </router-link>
    <!--Titulo-->
    <div class="card-body p-0 d-flex flex-column justify-content-around ">
        <router-link class="link" :to="{ name:'Serie',params:{id: id} }" :key="id" >
